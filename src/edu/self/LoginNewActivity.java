...
import com.bb.ui.MainActivity2;
...
public class LoginNewActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
		...
		.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) { 
				...
				if ((uin.length() == 0) || (pwd.length() == 0)) {
					Toast.makeText(LoginNewActivity.this, "用户名或者密码不能为空",
		...
		((Button) findViewById(R.id.login_option)).setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				startActivity(new Intent(LoginNewActivity.this, RegisterAccountActivity.class));
				finish();
			}
		});
    }
    ...
    public void login() {
    	...
    	new Thread(){
			public void run() {
				if (HttpUtil.isConnectInternet(LoginNewActivity.this)) {
								...
								if (content.equals("true")) {
									runOnUiThread(new Runnable() {
										public void run() {
											...
											intent.setClass(LoginNewActivity.this, MainActivity2.class);
									...
									runOnUiThread(new Runnable() {
										public void run() {
											dialog.dismiss();
											Toast.makeText(LoginNewActivity.this, "帐号不存在或用户名密码错误！", Toast.LENGTH_LONG).show();
										}
									});
								} else {
									runOnUiThread(new Runnable() {
										public void run() {
											dialog.dismiss();
											Toast.makeText(LoginNewActivity.this, "登录失败，请稍后再试！", Toast.LENGTH_LONG).show();
										}
									});
								}
							}
